...
*/
	...
	double data_stddev = 1e+1;		//The standard deviation of the data
...
ApplyState(sys,N,GlobalVars,prev_sys_state->ve,0.0,workspace);
*/
...
*/
		...
		{
			unsigned int loc = i / dim;
			unsigned int idx = i % dim;

			ApplyState(sys,N,GlobalVars,prev_sys_state->ve,t_b);	//Apply the state at time t_b
//printf("\nloc = %u idx = %u init = %.16e maxtime = %f\n",loc,idx,sys[loc]->list->head->y_approx->ve[idx],GlobalVars->maxtime);
			...
			//for(j=0;j<all_states;j++)	MT->me[j][i] = sys[j/dim]->list->tail->y_approx->ve[idx];	//!!!! Store M^T !!!!
//printf("Up %.16e %.16e\n",sys[0]->list->tail->y_approx->ve[0],sys[0]->list->tail->y_approx->ve[1]);
			...
			CheckConsistency(sys[loc]->list->head->y_approx,dim,GlobalVars->type,sys[loc]->params,GlobalVars->global_params);
			AsynchSolver(sys,N,my_sys,my_N,my_max_nodes,GlobalVars,assignments,id_to_loc,workspace,conninfo,my_data,0,outputfile);	//Solve system !!!! Can do this better !!!!
			//for(j=0;j<all_states;j++)	MT->me[j][i] = (MT->me[j][i] - sys[j/dim]->list->tail->y_approx->ve[idx])/(2.0*delta_x);	//!!!! Store M^T !!!!
//printf("Down %.16e %.16e\n",sys[0]->list->tail->y_approx->ve[0],sys[0]->list->tail->y_approx->ve[1]);
			for(j=0;j<all_states;j++)	MT->me[i][j] = (MT->me[i][j] - sys[j/dim]->list->tail->y_approx->ve[j%dim])/(2.0*delta_x);
//printf("delta_x = %.16e  derivs = %.16e %.16e\n",delta_x,MT->me[i][0],MT->me[i][1]);
//getchar();
		}
