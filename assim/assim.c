...
//For model 254 trim
    ...
    */
    ...
    {
        if (assignments[i] == my_rank)
            inv_upareas[i] = 1.0 / (sys[i].params.ve[GlobalVars->area_idx] * 1e3);
    }
    ...
    Asynch_Set_Total_Simulation_Time(&asynch, num_steps * obs_time_step);	//!!!! Is this needed? !!!!
