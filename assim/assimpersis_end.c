...
*/
	...
	if(my_rank == 0 && (atoi(argv[5]) - atoi(argv[4])) > 1e-4)
	{
		printf("Error: Non-zero simulation times not supported yet.\n");
		MPI_Abort(MPI_COMM_WORLD,1);
	}
	Asynch_Set_Total_Simulation_Time(asynch,(atoi(argv[5]) - atoi(argv[4]))/60.0);
	//Asynch_Set_Init_Timestamp(asynch,atoi(argv[7]));
	...
	//Check if there is new rainfall
	if(my_rank == 0)
	{
		isnull = 1;
		ConnectPGDB(asynch->db_connections[ASYNCH_DB_LOC_FORCING_START+forecast_idx]);
		sprintf(query,Forecaster->rainmaps_db->queries[0],atoi(argv[4])+ 60 * (unsigned int) rint(asynch->forcings[0]->file_time) * (Forecaster->num_rainsteps-1));
		//sprintf(query,"SELECT min(unix_time) FROM rain_maps5_index WHERE unix_time >= %u AND link_count > -1;",atoi(argv[4])+ 60 * (unsigned int) rint(asynch->forcings[0]->file_time) * (Forecaster->num_rainsteps-1));
	...
	AssimData* Assim = Init_AssimData(argv[3],asynch);
	if(!Assim)	MPI_Abort(MPI_COMM_WORLD,1);
	if(Download_Gauge_IDs(asynch,Assim))	MPI_Abort(MPI_COMM_WORLD,1);
	
	//Check if there are new observations
if(my_rank == 0)
printf("Using %u %u\n",atoi(argv[4]) - (Assim->steps_to_use - 1) * (unsigned int)(Assim->inc*60.0+1e-3),atoi(argv[4]));
	isnull = GaugeDataAvailable(Assim,atoi(argv[4]) - (Assim->steps_to_use - 1) * (unsigned int)(Assim->inc*60.0+1e-3),atoi(argv[4]) + (unsigned int)(Assim->inc*60.0+1e-3));
	if(isnull)
	{
		unsigned int btime = atoi(argv[4]) - (Assim->steps_to_use - 1) * (unsigned int)(Assim->inc*60.0+1e-3);
		unsigned int etime = atoi(argv[4]) + (unsigned int)(Assim->inc*60.0+1e-3);
		if(my_rank == 0)	printf("Not enough new observations (%u to %u). Exiting...\n",btime,etime);
		//Asynch_Free(asynch);	!!!! This needs to be fixed !!!!
		MPI_Finalize();
		return 0;
	}

	//Make sure the total time is large enough for creating the temporary files
	...
	//if(Download_Gauge_IDs(asynch,Assim))	MPI_Abort(MPI_COMM_WORLD,1);
	Find_Upstream_Links(asynch,problem_dim,1,Assim->inc,Assim->steps_to_use,Assim->data_locs,Assim->numdata);
	...
	for(i=0;i<asynch->GlobalVars->num_forcings;i++)
	{
		...
		//	Asynch_Set_Last_Rainfall_Timestamp(asynch,override_endtime,i);
	}
	...
	//Initialize choices
	unsigned int numdata = Assim->numdata,*data_locs = Assim->data_locs,steps_to_use = Assim->steps_to_use;
	...
	double* d_full = calloc(numdata*steps_to_use,sizeof(double));
...
printf("allstates_needed: %u allstates: %u\n",allstates_needed,allstates);
	...
	if(my_rank == 0)
	{
		...
		MatCreateSeqDense(MPI_COMM_SELF,allstates_needed,numdata*steps_to_use,NULL,&HMTR);
	...
	int* entries_d = (int*) malloc(steps_to_use*numdata*sizeof(int));
	for(i=0;i<steps_to_use*numdata;i++)	entries_d[i] = i;
	...
	if(my_rank == 0)
	{
		...
		VecCreateSeq(MPI_COMM_SELF,numdata*steps_to_use,&R);
		for(i=0;i<numdata;i++)
		{
			for(j=0;j<steps_to_use;j++)
				VecSetValue(R,j*numdata + i,invupareas[data_locs[i]] * 10.0,INSERT_VALUES);
		}
		VecAssemblyBegin(R);
		VecAssemblyEnd(R);
	}
...
/*
	...
	if(my_rank == 0)
	{
		...
		{
			for(j=0;j<steps_to_use;j++)
				VecSetValue(R,j*steps_to_use + i,invupareas[data_locs[i]] * 10.0,INSERT_VALUES);
		}
		VecAssemblyBegin(R);
		VecAssemblyEnd(R);
	}
...
*/
	...
	user.inc = inc;
	user.steps_to_use = Assim->steps_to_use;
...
printf("I see %u %u\n",asynch->forcings[forecast_idx]->last_file,change_time);

	//Setup temp files
	Set_Output_User_forecastparams(asynch,forecast_time_unix,simulation_time_with_data_secs);	//!!!! Should this be done here at all? !!!!
	Set_Output_PeakflowUser_Offset(asynch,forecast_time_unix);	//!!!! Does this need to be here? !!!!
	...
	if(my_rank == 0)
	{
		...
		//Clear future hydrographs
		sprintf(query,"DELETE FROM master_archive_hydroforecast_%s WHERE forecast_time > %u;",Forecaster->model_name,forecast_time_unix);
		res = PQexec(asynch->db_connections[ASYNCH_DB_LOC_HYDRO_OUTPUT]->conn,query);
		if(CheckResError(res,"deleting future forecasts"))
			MPI_Abort(MPI_COMM_WORLD,1);
...
/*
		...
		DeleteFutureValues(asynch->db_connections[ASYNCH_DB_LOC_HYDRO_OUTPUT],num_tables,asynch->GlobalVars,"archive_hydroforecast",Forecaster->model_name,forecast_time_unix,1);
*/
/*
...
//printf("!!!! Skipping initialization. Skipping maintenance too. !!!!\n");
	...
	while(!halt)
	{
		...
		background_time_unix = forecast_time_unix - (Assim->steps_to_use - 1) * (unsigned int)(Assim->inc*60.0+1e-3);
...
/*
		...
		//if(k % 96 == 0)
		if(forecast_time_unix % (4*24*3600) == 0)
		{
...
printf("Making query %s\n",query);
			...
			{
				time(&start);
				isnull_obs = GaugeDataAvailable(Assim,background_time_unix,forecast_time_unix+(unsigned int)(Assim->inc*60.0+1e-3));
				if(my_rank == 0)
				{
					time(&stop);
					printf("Total time to check for new observations: %f.\n",difftime(stop,start));
				}
			}

			if(isnull || isnull_obs)
			{
				if(my_rank == 0)
				{
					if(isnull)	printf("No rainfall values returned from SQL database for forcing %u. %u %u\n",forecast_idx,forecast_time_unix,isnull);
					else		printf("Not enough new observations (%u to %u). Exiting...\n",background_time_unix,forecast_time_unix);
...
*/
		...
		if(halt || isnull || isnull_obs)	break;
...
*/
	...
	//Make a snapshot
	Asynch_Get_Snapshot_Output_Name(asynch,filename);
	sprintf(&(filename[strlen(filename)-4]),"%s",".ini");
	Asynch_Set_Snapshot_Output_Name(asynch,filename);
	SnapShot_ModelStates(asynch,problem_dim);

	//Save the last timestep
	if(my_rank == 0)
	{
		FILE* exitfile = fopen(argv[6],"w");
		//fprintf(exitfile,"%i",first_file);
		fprintf(exitfile,"%i",forecast_time_unix);
		fclose(exitfile);
	}
...
void Make_Assimilated_Forecasts(asynchsolver* asynch,unsigned int background_time_unix,double simulation_time_with_data,VEC** backup,AppCtxFull* user,ForecastData* Forecaster,AssimData* Assim,unsigned int assim_dim,unsigned int forecast_idx)
{
	...
	int *assignments = asynch->assignments;
	short int *getting = asynch->getting;
	unsigned int **id_to_loc = asynch->id_to_loc,N = asynch->N,numdata = Assim->numdata;
...
time(&q_start);
		...
		while(GetObservationsDB(Assim,id_to_loc,N,background_time_unix,d_full) == -1)
		{
...
/*
		...
		for(j=0;j<(steps_to_use-1)*numdata;j++)
...
{
	AdjustDischarges_Scale(asynch,Assim->data_locs,d_full,numdata,x_b,allstates,assim_dim);
}
*/
...
*/
		...
		do
		{
			...
			//while(diff > 1e-2)
			{
				...
				error = compute_diff(d_full,q,steps_to_use*numdata);
				if(prev_error >= 0.0)
				{
			...
			{
				//try_again = 1;
				try_again = ReduceBadDischargeValues(sys,assignments,N,d_full,q,steps_to_use,user->data_locs,numdata,x_start,assim_dim,1.0);	//!!!! Not sure what to use for the limit... !!!!
			}
			else	try_again = 0;
		}while(try_again);
...
/*
	...
	if(i < steps_to_use)
	{
printf("Starting at %u, going to %u\n",i*numdata,steps_to_use*numdata);
		for(j=0;j<allstates;j++)
			HM_els[j] = 0.0;
		for(i=i*numdata;i<steps_to_use*numdata;i++)
...
time(&start2);
		...
		for(i=0;i<max_or_steps*numdata;i++)	//If negative, something bad happened to the observations. So assume the model is perfect.
		{
			if(d_tmp[i] < 0.0)	d_tmp[i] = 0.0;
			else			d_tmp[i] = d_tmp[i] - q[i];
		}
