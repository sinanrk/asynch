...
//	*my_N = end_index - start_index;
	...
	*my_sys = (unsigned int*) malloc(my_max_nodes * sizeof(unsigned int));	//The indices of this processes links (in sys)
	...
	for(i=0;i<N;i++)	vwgt[i] = ((upstream_data*)(sys[i]->user))->dim;
	if(np != 1)
	{
...
//This is used to remove the sensitivites that are not used.
unsigned int BuildStateShift(asynchsolver* asynch,unsigned int allstates,unsigned int* data_locs,unsigned int numdata,unsigned int** vareq_shift,unsigned int** inv_vareq_shift)
{
	...
	unsigned int *needed = (unsigned int*) calloc(allstates,sizeof(unsigned int));
	...
	*inv_vareq_shift = (unsigned int*) calloc(allstates,sizeof(unsigned int));
	...
	for(i=0;i<numdata;i++)
	{
		...
		{
			updata = (upstream_data*) sys[data_locs[i]]->user;
			for(j=0;j<updata->num_fit_states;j++)
				needed[updata->fit_to_universal[j]] = 1;
		}
	}

	MPI_Allreduce(needed,*vareq_shift,allstates,MPI_UNSIGNED,MPI_LOR,MPI_COMM_WORLD);

	//Compute the shifts
	unsigned int shift = 0;
	for(i=0;i<allstates;i++)
	{
		if((*vareq_shift)[i] == 0)	shift++;
		else
		{
...
void InitRoutines_Assim_254(Link* link,unsigned int type,unsigned int exp_imp,unsigned short int dam,void* external)
{
	...
	for(i=0;i<link->numparents;i++)
		link->dim += data->num_upstream[i] * problem_dim;	//Variational eqs from upstream
	//if(link->dim != data->dim)
	//	printf("[%i]: Warning: calculated the number of equations at link %u twice and got %u and %u.\n",my_rank,link->ID,link->dim,data->dim);
	link->no_ini_start = 4;
	link->diff_start = 0;
...
int ReadInitData_Assim_254(VEC global_params,VEC params,QVSData* qvs,unsigned short int dam,VEC y_0,unsigned int type,unsigned int diff_start,unsigned int no_init_start,void* user,void* external)
{
	...
	for(i=offset;i<y_0.dim;i++)	y_0.ve[i] = 0.0;	//From upstream
...
void TopLayerHillslope_extras_assim(double t,VEC y_i,VEC* y_p,unsigned short int numparents,VEC global_params,double* forcing_values,QVSData* qvs,VEC params,int state,void* user,VEC ans)
{
	...
	upstream_data *updata = (upstream_data*) user;
	...
	//Compute partial derivatives (upstream variables)
	...
	for(i=0;i<numparents;i++)
	{
		//parent_idx = offset + 8;
		parent_idx = offset + 7;
		for(j=0;j<updata->num_upstream[i];j++)
		{
...
void Setup_Fitting_Data_Model254(asynchsolver* asynch,unsigned int* data_locs,unsigned int numdata)
{
	...
	for(i=0;i<numdata;i++)
	{
		if(assignments[data_locs[i]] == my_rank)
		{
			...
			for(j=0;j<current->numparents;j++)
				updata->num_fit_states += updata->num_upstream[j];
...
/*
			...
			for(j=0;j<current->numparents;j++)
			{
				for(k=0;k<updata->num_upstream[j];k++)
				{
					...
					updata->fit_to_universal[counter] = updata->upstream[j][k] * assim_dim;
					counter++;
				}
			}
*/
			...
			for(j=0;j<current->numparents;j++)
			{
				for(k=0;k<updata->num_upstream[j];k++)
				{
					...
					updata->fit_to_universal[counter+1] = updata->upstream[j][k] * assim_dim + 1;
					counter += 2;
				}
			}
...
void InitRoutines_Assim_254_q(Link* link,unsigned int type,unsigned int exp_imp,unsigned short int dam,void* external)
{
	...
	for(i=0;i<link->numparents;i++)
		link->dim += data->num_upstream[i];	//Variational eqs from upstream
	link->no_ini_start = 4;
	link->diff_start = 0;
...
void TopLayerHillslope_assim_q(double t,VEC y_i,VEC* y_p,unsigned short int numparents,VEC global_params,double* forcing_values,QVSData* qvs,VEC params,int state,void* user,VEC ans)
{
	...
	//Discharge variational eqs from parent links
	unsigned int current_idx = offset + 1,parent_idx;
	for(i=0;i<numparents;i++)
	{
		parent_idx = offset;
		for(j=0;j<updata->num_upstream[i];j++)
...
void InitRoutines_Assim_254_qsp(Link* link,unsigned int type,unsigned int exp_imp,unsigned short int dam,void* external)
{
	...
	for(i=0;i<link->numparents;i++)
		link->dim += data->num_upstream[i] * 2;	//Variational eqs from upstream
	link->no_ini_start = 4;
	link->diff_start = 0;
...
/*
printf("ID = %u\n",current->ID);
for(k=0;k<current->numparents;k++)
	printf("upstream = %u\n",updata->num_upstream[k]);
		...
		if(assignments[data_locs[i]] == my_rank)
		{
			...
			for(j=0;j<current->numparents;j++)
			{
				for(k=0;k<updata->num_upstream[j];k++)
				{
					...
					updata->fit_to_universal[counter] = updata->upstream[j][k] * assim_dim;
					updata->fit_states[counter+1] = problem_dim + 4 + counter;
					updata->fit_to_universal[counter+1] = updata->upstream[j][k] * assim_dim + 2;
					counter += 2;
				}
			}
/*
